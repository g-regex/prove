...
Notice that statement~8 cannot be verified. It now has the \texttt{NEWC}
flag set, indicating that a new constant has been introduced -\ it would be
verifiable when adding one additional assumption: \m{[c=a]}.
\newline

\discuss{%
	We might want to change this behaviour and allow new identifiers to be
	introduced anywhere in the code (i.e.\ also after a \m{=>} formulator).
	Then, Source~Code~\ref{code:var1} could successfully be verified - but for a
	different reason as if we would have used the suggested additional
	assumption.
}
...
\example{var2v1}
\discuss{%
	Here \m{[[a]=[c]]} is part of the initial assumptions. The verification
	is successful as node~14 (\m{[a]}) is found to be similar both nodes 11 and
	8, which are equal by the statement of node~7.
}
