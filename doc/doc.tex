...
\begin{figure}[!ht]
\centering
\textcolor{dartmouthgreen}{\input{ebnf}}
...
\begin{code}
\captionof{listing}{\name\ output for
Source~Code~\ref{code:var1}}\label{code:var1out}
\inputminted[linenos]{\prv}{examples/out/var1.out}
\end{code}
...
verifyable when adding two additional assumptions: \m{[a]} and \m{[c=a]}.
\newline

\textcolor{dartmouthgreen}{%
We might want to change this behaviour and allow new identifiers to be
introduced anywhere in the code (i.e.\ also after a \m{=>} formulator). Then
however, wee will have to agree on rules, under which conditions we are allowed
to conclude an equality. The next pages show different approaches.}
...
\begin{code}
\captionof{listing}{Example}\label{code:var2}
\inputminted[linenos]{\prv}{examples/var2.prove}
\end{code}

\begin{figure}[!h]
\caption{Representation of source~code~\ref{code:var2}}\label{fig:var2}
\centering
\subfloat[\centering Legend]{%
	\includegraphics[width=\textwidth, height=8cm,
	keepaspectratio]{tikz/legend.pdf}
}
\subfloat[\centering Graph of source~code~\ref{code:var2}]{%
	\includegraphics[width=\textwidth, height=8cm,
	keepaspectratio]{tikz/var2.pdf}
}
\end{figure}

\begin{code}
\captionof{listing}{\name\ output for
Source~Code~\ref{code:var2}}\label{code:var2out}
\inputminted[linenos]{\prv}{examples/out/var2.out}
\end{code}

\textcolor{dartmouthgreen}{%
Some discussion\dots}
