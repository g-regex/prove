...
verifiable when adding two additional assumptions: \m{[a]} and \m{[c=a]}.
\newline
...
\discuss{%
	...
	fails again, since node~8 is treated as a variable, when trying to verify
	node~14 (notice the \texttt{VAR} flag of node~7).
}
...
\example{var3v1}
\discuss{%
	Similar problems as in Source~Code~\ref{code:var2v1} arise here.
}
...
\example{var4}
\discuss{%
	Here, nodes 8 and 10 cannot be verified.
}
