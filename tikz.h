...
#define TIKZ_HEADER \
	...
	"\\node (IMPL) at (0pt,0pt) {"\
	"\\texttt{IMPL}: node is part of an implication};\n"\
	"\\node[right = " TIKZ_LGND_SPACING "pt of IMPL] (EQTY)"\
	"{\\texttt{EQTY}: node is part of an equality};\n"\
	"\\node[right = " TIKZ_LGND_SPACING "pt of EQTY] (FMLA)"\
	"{\\texttt{FMLA}: node is part of an ordinary formula};\n"\
	"\\node[right = " TIKZ_LGND_SPACING "pt of FMLA] (ASMP)"\
	"{\\texttt{ASMP}: node is an assumption};\n"\
	...
	"{\\texttt{FRST}: node is positioned before first occurrence of an "\
	"implication formulator in that formula};\n"\
