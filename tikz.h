...
#define TIKZ_COLOR8 "brightpink"
#define TIKZ_COLOR8_RGB "1.0, 0.0, 0.5"
#define TIKZ_COLOR9 "black"
#define TIKZ_COLOR9_RGB "0.0, 0.0, 0.0"
...
#define TIKZ_LGND_ASMP "\\texttt{ASMP}: appearance of the node at the current "\
...
#define TIKZ_LGND_TRUE "\\texttt{TRUE}: statement is true"
#define TIKZ_LGND_VAR "\\texttt{VAR}: subtree contains at least one variable"
#define TIKZ_LGND_INACT "flag is not applicable to that node (but set due "\
	"to efficiency related considerations)"
...
#define TIKZ_HEADER \
	...
	"\\definecolor{" TIKZ_COLOR8 "}{rgb}{" TIKZ_COLOR8_RGB "}\n"\
	"\n\\usepackage{calc}\n"\
...
#define TIKZ_LGND_P(p1, p2) \
	"\n%%Legend\n"\
	"\\sbox{\\tlegend}{\n"\
	"\\resizebox{0.9\\hsize}{!}{"\
	"\\begin{tikzpicture}[node distance = 1pt, auto]\n"\
	"\\node[" p1 "] (IMPL) at (0pt,0pt) {" TIKZ_LGND_IMPL "};\n"\
	"\\node[" p1 p2 " of IMPL] (EQTY)"\
	"{" TIKZ_LGND_EQTY "};\n"\
	"\\node["  p1 p2 " of EQTY] (FMLA)"\
	"{" TIKZ_LGND_FMLA "};\n"\
	"\\node[" p1 p2 " of FMLA] (ASMP)"\
	"{" TIKZ_LGND_ASMP "};\n"\
	"\\node[" p1 p2 " of ASMP] (NEWC)"\
	"{" TIKZ_LGND_NEWC "};\n"\
	"\\node[" p1 p2 " of NEWC] (LOCK)"\
	"{" TIKZ_LGND_LOCK "};\n"\
	...
	"to (VAR.west);\n"\
	"\\draw[-{Triangle[length=" TIKZ_LGND_ASIZE "pt,width=" TIKZ_LGND_ASIZE \
	"pt]}, color=" TIKZ_COLOR_INACT "] ([xshift=-" TIKZ_LGND_ASIZE "pt] INACT.west)"\
	"to (INACT.west);\n"\
	"\\end{tikzpicture} } }\n"
#define TIKZ_LGND TIKZ_LGND_P("", "right = " TIKZ_LGND_SPACING "pt") 
#define TIKZ_LGND_V TIKZ_LGND_P("text width = 280pt, align = left, ", "below = " TIKZ_LGND_SPACING "pt") 
...
#define TIKZ_FLAG_B(pnum, shift) \
	"] ([yshift=%1.1fpt] %d.east) to ([yshift=%1.1fpt, xshift=%dpt] "\
	"%d.east);\n", TIKZ_FLAG_SIZE, TIKZ_FLAG_SIZE,\
	(((double) TIKZ_NUM_FLAGS - 1) / 2 - shift) * TIKZ_FLAG_SIZE, pnum,\
...
/* To include the legend in the graph output, use TIKZ_TOOTER_WITH_LGND
 * in proveparser.c */
